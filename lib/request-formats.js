...
exports.shipConfirm = function(data) {
    ...
    response += "<ShipmentConfirmRequest>";
		...
		response += "<Shipment>";
			...
			if(!data.shipment.shipper) return false;
			...
			response += "<Shipper>";
				...
				if(!shipper.address) return false;
				
				response += buildAddress(shipper.address);

			response += "</Shipper>";

			if(!data.shipment.shipTo) return false;
			...
			response += "<ShipTo>";
				...
				if(!shipTo.address) return false;
			...
			if(!data.shipment.payment) return false;
			...
			if(!data.shipment.service) return false;
			...
			if(!data.shipment.package) return false;
